...
func evaluateLokiRule(lokiRule *loggingv1beta1.LokiRule) error {
	for _, group := range lokiRule.Spec.Groups {
		for _, rule := range group.Rules {
			_, err := logql.ParseExpr(rule.Expr)
			if err != nil {
				return fmt.Errorf("%s: %s", rule.Alert, err.Error())
			}
		}
	}
